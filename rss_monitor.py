...
for feed in rss_feeds:
    ...
    if feed_data.entries:
        ...
        published_str = latest_entry.get(
            "published") or latest_entry.get("pubDate")
        print(f"published_str : {published_str}")
        if published_str:
            try:
                # Parse the published time and convert to UTC
                published_time = parsedate_to_datetime(
                    published_str).astimezone(timezone.utc)
                ...
                # Check if the time difference is within the last 5 minutes
